...
if exist "%SEEDANCE_MANIFEST%" (
  "%PYTHON%" "%PROJECT_ROOT%\scripts\train_wangxing_specialization.py" %* --seedance-label-manifest "%SEEDANCE_MANIFEST%"
) else (
  echo Warning: Seedance pseudo-label manifest is missing.
  echo Run scripts\run_seedance_expression_labeling.cmd first.
  "%PYTHON%" "%PROJECT_ROOT%\scripts\train_wangxing_specialization.py" %*
)
