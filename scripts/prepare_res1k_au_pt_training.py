...
def write_commands_file(path: Path) -> None:
    text = f"""# AU+.pt 分辨率增强重训（test/AI 五条只测不训，长边 >= {MIN_LONG_EDGE}）
...
## 1) 双尺度 .pt（新路径，不覆盖旧 dual）

AU 学习头沿用现有 noleak，不必为这 5 条重训。
...
.\\.venv\\Scripts\\python.exe scripts\\run_wangxing_specialization_fused.py evaluate `
  ...
  --learned-head outputs\\forensics\\learned_fusion_head_logistic_noleak.json `
  --pt-model outputs\\vedio_pred\\models\\wangxing_dual_scale_classifier_res1k.pt `
  --pt-cache-dir outputs\\vedio_pred\\cache_res1k `
  --output outputs\\forensics\\wangxing_specialization_fused_holdout_metrics_res1k.json
```
...
.\\.venv\\Scripts\\python.exe scripts\\run_wangxing_specialization_fused.py evaluate `
  ...
  --pt-model outputs\\vedio_pred\\models\\wangxing_dual_scale_classifier_res1k.pt `
  --pt-cache-dir outputs\\vedio_pred\\cache_res1k `
  --output outputs\\forensics\\wangxing_specialization_fused_test_AI_metrics_res1k.json
```
...
- 未改默认 dual `.pt` / noleak 头 / `forensics_profiles.json`
"""
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
