...
def _align_ground_truth_frame(
    ...
    *,
    ...
    aligned = _resize_like(frame, target)
    metadata = {
        "mode": "resize_gt_to_result",
        "source_size": {
...
def evaluate_full_reference(
    ...
    alignment_mode = "resize_gt_to_result"
    ...
    if result_info.width != ground_truth_info.width or result_info.height != ground_truth_info.height:
        warnings.append(
            "GT 分辨率或宽高比与结果视频不同，计算前已将 GT 帧 resize 到结果视频分辨率。"
        )
    if result_info.fps > 0 and ground_truth_info.fps > 0:
    ...
    return {
        ...
        "alignment": {
            ...
            "comparison_region": "full_frame",
            "resize": alignment_metadata,
        },
        "metrics": {
            "psnr_db": (
