...
function renderQueue() {
  ...
  for (const item of visible) {
    ...
    button.innerHTML = `
      ...
      <small>${text(item.category)} · ${text(item.sample_id)}</small>
...
function renderDetail(payload) {
  ...
  detailNode.innerHTML = `
    ...
    <p>${text(item.category)} · ${text(item.sample_id)} · ${text(item.published_at)}</p>
    <div class="headline">
      ${metric("真实性概率", percent(probability))}
      ${metric("身份证据", percent(identity))}
      ${metric("表情证据", percent(expressionScore))}
      ${metric("取证证据", percent(forensicsScore))}
    </div>
    <div class="conclusion"><strong>${text(conclusion)}</strong><br>
